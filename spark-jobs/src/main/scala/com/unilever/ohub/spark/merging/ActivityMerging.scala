...
object ActivityMerging extends SparkJob[ActivityMergingConfig] {
  ...
  def transform(
    ...
    previousIntegrated
      ...
      .map {
        case (activity: Activity, cpn: ContactPerson) => activity.copy(contactPersonOhubId = cpn.ohubId)
        case (activity, _) => activity
      }
      // update opr ids
      .joinWith(operators, $"operatorConcatId" === operators("concatId"), "left")
      .map {
        case (activity: Activity, opr: Operator) => activity.copy(operatorOhubId = opr.ohubId)
        case (activity, _) => activity
      }
  }
