...
abstract class CsvDomainGateKeeper[DomainType <: DomainEntity : TypeTag] extends DomainGateKeeper[DomainType, Row, CsvDomainConfig] {
  ...
  override protected def read(spark: SparkSession, storage: Storage, config: CsvDomainConfig): Dataset[Row] = {
    ...
    if (numberOfFields == 1) {
      log.error(
        s"""
           |Number of fields in schema is '$numberOfFields',
           |probably the fieldSeparator is specified incorrectly, currently it's '$fieldSeparator',
           |which resulted in the following schema: ${result.schema}.
           |"""
          .stripMargin
      )
      System.exit(1)
