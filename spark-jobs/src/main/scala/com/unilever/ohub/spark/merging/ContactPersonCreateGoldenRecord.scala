...
object ContactPersonCreateGoldenRecord extends BaseMerging[ContactPerson] {
  ...
  private[merging] def pickLatestConsent(spark: SparkSession,
    ...
    println("within consent function column " + column)

    val groupWindow = Window.partitionBy($"ohubId")
    val consentOrderDateColumn = consentFlagDateMapping(column)

    println("Sorting date " + consentOrderDateColumn + " for the column " + column)

    val consentOrderWindow = groupWindow.orderBy(
      col(consentOrderDateColumn).desc_nulls_last,
    ...
    df.withColumn(
      consentOrderDateColumn, when(
        col(column).isNull, Timestamp.valueOf("1970-01-01 00:00:00")
      ).otherwise(col(consentOrderDateColumn))
    )
      .withColumn(prefixNewColumn + column, first(col(column), true).over(consentOrderWindow.rowsBetween(Window.unboundedPreceding, Window.unboundedFollowing)))
      .withColumn(prefixNewColumn + consentOrderDateColumn, first(col(consentOrderDateColumn), true).over(consentOrderWindow.rowsBetween(Window.unboundedPreceding, Window.unboundedFollowing)))
    ...
    var dataframeModified: DataFrame = columns
      .foldLeft(dataframe)(
        (cp: DataFrame, column: String) ⇒ {
          ...
          else if (skipColumns.contains(column)) {println("skipping"+column); cp} //Skip the consent Date columns. Elaborated with Consent Flag
