...
object OperatorDispatchConverter extends Converter[Operator, DispatchOperator] with TypeConversionFunctions with DispatchTransformationFunctions {
  ...
  override def convert(implicit operator: Operator, explain: Boolean = false): DispatchOperator = {
    DispatchOperator(
      ...
      DM_OPT_OUT = getValue("hasDirectMailOptOut", BooleanToYNConverter),
      EMAIL_OPT_OUT = getValue("hasEmailOptOut", BooleanToYNConverter),
      FIXED_OPT_OUT = getValue("hasTelemarketingOptOut", BooleanToYNConverter),
      MOBILE_OPT_OUT = getValue("hasMobileOptOut", BooleanToYNConverter),
      FAX_OPT_OUT = getValue("hasFaxOptOut", BooleanToYNConverter),
