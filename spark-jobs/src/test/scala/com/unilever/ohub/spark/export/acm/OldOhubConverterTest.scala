...
class OldOhubConverterTest extends FunSpec {
  ...
  describe("When source is unknown") {
    it("Should leave out the source id") {
      ...
      assert(new OperatorOldOhubConverter(sourceIds).convert("a~b~c") == "a~c~1~")
    }
  }
  ...
  describe("When source is known") {
    ...
    it("Should add sourceId") {
      assert(new OperatorOldOhubConverter(sourceIds).convert("a~FILE~c") == "a~c~1~122")
    }
    it("Should still work when source id contains a ~") {
      assert(new OperatorOldOhubConverter(sourceIds).convert("ES~FILE~O~247087") == "ES~O~247087~1~122")
    }
  }
}
