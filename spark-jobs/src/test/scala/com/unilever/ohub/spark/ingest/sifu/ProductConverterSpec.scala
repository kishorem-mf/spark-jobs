...
import com.unilever.ohub.spark.ingest.ProductEmptyParquetWriter

class ProductConverterSpec extends SifuDomainGateKeeperDatasetSpec[Product] with TestProducts {

  override val SUT = new SifuProductConverter with ProductEmptyParquetWriter {
    ...
    it("should convert a product correctly from a valid api input") {
      ...
      runJobWith() { actualDataSet ⇒
        ...
        val expectedProduct = defaultProduct.copy(
          ...
          nutrientValues = List("67 kcal", "0.5 g", "0.1 g", "8.1 g", "4.0 g", "1.5 g", "7.8 g", "8,200 mg"),
          orderScore = Some(1),
