...
class LoyaltyPointsMergingSpec extends SparkJobSpec with TestLoyaltyPoints with TestContactPersons with TestOperators {
  ...
  describe("LoyaltyPoints merging") {
    ...
    it("should pick proper golden record") {
      ...
      val duplicateRecord = defaultLoyaltyPoints.copy(
        ohubId = Some("oldId"),
        contactPersonConcatId = Some("AU~WUFOO~cpn1"),
        concatId = s"${defaultLoyaltyPoints.sourceName}~${defaultLoyaltyPoints.sourceEntityId}")

      val duplicateRecordForLP = defaultLoyaltyPoints.copy(
        ohubId = Some("oldId"),
        contactPersonConcatId = Some("AU~WUFOO~cpn1"),
        concatId = s"${defaultLoyaltyPoints.sourceName}~${defaultLoyaltyPoints.sourceEntityId}",
        isActive = true)

      val newRecordForLP = defaultLoyaltyPoints.copy(
        ohubId = Some("newId"),
        contactPersonConcatId = Some("AU~WUFOO~cpn1"),
        concatId = s"new-${defaultLoyaltyPoints.sourceName}~${defaultLoyaltyPoints.sourceEntityId}")
      ...
      val previous: Dataset[LoyaltyPoints] = spark.createDataset(Seq(
        duplicateRecord,
        duplicateRecordForLP
      ))

      val input: Dataset[LoyaltyPoints] = spark.createDataset(Seq(
        duplicateRecord.copy(ohubId = Some("newId"), dateUpdated = Some(Timestamp.valueOf(LocalDateTime.now))),
        newRecordForLP
      ))
      ...
      result.collect()(0).contactPersonOhubId shouldBe Some("ohubCpn1")
      result.collect()(0).ohubId should not be Some("newId")
    }
  }
}
