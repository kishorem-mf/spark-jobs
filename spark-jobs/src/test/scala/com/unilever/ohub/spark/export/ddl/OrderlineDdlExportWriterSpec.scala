...
class OrderlineDdlExportWriterSpec extends SparkJobSpec with TestOrderLines with BeforeAndAfter with Matchers {

  import spark.implicits._


  private val orderlines = Seq(defaultOrderLine).toDataset
  private val integrated = Seq(defaultOrderLine).toDataset
  private val SUT = com.unilever.ohub.spark.export.ddl.OrderlineDdlOutboundWriter
  ...
  val storage = new InMemStorage(spark, orderlines, integrated)
  ...
  describe("DDL Orderline csv generation") {
    it("DDL Orderline csv generation") {
      ...
      val integratedDs = Seq(
        defaultOrderLine.copy(concatId = "AU~WUFOO~101", ohubId = Some("1"), isGoldenRecord = true, sourceName = "FRONTIER"),
        defaultOrderLine.copy(concatId = "AU~WUFOO~102", ohubId = Some("3"), isGoldenRecord = true, sourceName = "FRONTIER"),
        defaultOrderLine.copy(concatId = "AU~WUFOO~104", ohubId = Some("2"), isGoldenRecord = true, sourceName = "FRONTIER")
      ).toDataset
