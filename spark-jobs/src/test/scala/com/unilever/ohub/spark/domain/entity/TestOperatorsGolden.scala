...
object TestOperatorsGolden extends TestOperatorsGolden

trait TestOperatorsGolden {

  def defaultOperatorWithSourceName(sourceName: String): OperatorGolden =
    defaultOperatorGolden.copy(
      sourceName = sourceName,
      concatId = Util.createConcatIdFromValues(defaultOperatorGolden.countryCode, sourceName, defaultOperatorGolden.sourceEntityId)
    )

  def defaultOperatorWithSourceNameAndCountryCode(source: String, countryCode: String): OperatorGolden =
    defaultOperatorGolden.copy(
      sourceName = source,
      countryCode = countryCode,
      concatId = Util.createConcatIdFromValues(countryCode, source, defaultOperatorGolden.sourceEntityId)
    )

  def defaultOperatorWithSourceEntityId(sourceEntityId: String): OperatorGolden =
    defaultOperatorGolden.copy(
      sourceEntityId = sourceEntityId,
      concatId = Util.createConcatIdFromValues(defaultOperatorGolden.countryCode, defaultOperatorGolden.sourceName, sourceEntityId)
    )

  // format: OFF
  lazy val defaultOperatorGolden: OperatorGolden = OperatorGolden(
    id                          = "id-1",
