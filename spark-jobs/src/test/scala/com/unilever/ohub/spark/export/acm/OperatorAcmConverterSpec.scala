...
class OperatorAcmConverterSpec extends SparkJobSpec with TestOperators {
  ...
  private val defaultTestOperator: AcmOperator = AcmOperator(
  ...
  private val defaultOperatorToConvert: Operator = defaultOperator.copy(
    ...
    sourceName = "FACEBOOK,GAZPACHO"
  )

  describe("operator acm converter") {
    it("should convert a domain operator correctly into an acm converter when all sources are available") {

      val operatorToConvert = defaultOperatorToConvert.copy(sourceName = "FACEBOOK,GAZPACHO")
      val result = SUT.convert(operatorToConvert)
      result shouldBe defaultTestOperator.copy(SOURCE_IDS = "-51-54-")
    }

    it("should convert a domain operator correctly into an acm converter when all sources aren't available") {

      val operatorToConvert = defaultOperatorToConvert.copy(sourceName = "NON-EXISTING-SOURCE")
      val result = SUT.convert(operatorToConvert)
      result shouldBe defaultTestOperator.copy(SOURCE_IDS = "")
    }

    it("should convert a domain operator correctly into an acm converter when some sources aren't available") {
      val operatorToConvert = defaultOperatorToConvert.copy(sourceName = "FACEBOOK,NON-EXISTING-SOURCE")
      val result = SUT.convert(operatorToConvert)
      result shouldBe defaultTestOperator.copy(SOURCE_IDS = "-51-")
    }
  }
