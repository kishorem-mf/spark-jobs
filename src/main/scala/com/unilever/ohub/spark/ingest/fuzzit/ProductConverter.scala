package com.unilever.ohub.spark.ingest.fuzzit
...
object ProductConverter extends FuzzitDomainGateKeeper[Product] with ProductEmptyParquetWriter {
  ...
  override def toDomainEntity: DomainTransformer ⇒ Row ⇒ Product = { transformer ⇒ row ⇒
    ...
    val countryCode                                   =   dataProvider.countrySalesOrg(salesOrg).countryCode
    val sourceEntityId                                =   mandatoryValue(MAT_UID, "sourceEntityId")(row)
