...
//  TODO make sure OPR_LNKD_INTEGRATION_ID = filled with new group id + set GOLDEN_RECORD_FLAG to Y if golden record
  val operatorFieldsParquetDF = spark.sql(
    """
      |select OPERATOR_CONCAT_ID OPR_ORIG_INTEGRATION_ID,null OPR_LNKD_INTEGRATION_ID,null GOLDEN_RECORD_FLAG,COUNTRY_CODE,
      | clean(NAME) NAME,CHANNEL,SUB_CHANNEL,null ROUTE_TO_MARKET,REGION,OTM,
      ...
      | case when 52 - WEEKS_CLOSED < 0 then null else 52 - WEEKS_CLOSED end NUMBER_OF_WEEKS_OPEN,DAYS_OPEN NUMBER_OF_DAYS_OPEN,CONVENIENCE_LEVEL,SALES_REP RESPONSIBLE_EMPLOYEE,NPS_POTENTIAL,null CAM_KEY,null CAM_TEXT,null CHANNEL_KEY,null CHANNEL_TEXT,CHAIN_ID CHAIN_KNOTEN,
      | clean(CHAIN_NAME) CHAIN_NAME,null CUST_SUB_SEG_EXT,null CUST_SEG_EXT,null CUST_SEG_KEY_EXT,null CUST_GRP_EXT,null PARENT_SEGMENT,DATE_CREATED,DATE_MODIFIED DATE_UPDATED,
      ...
      | null LOCAL_CHANNEL,null CHANNEL_USAGE,null SOCIAL_COMMERCIAL,null STRATEGIC_CHANNEL,null GLOBAL_CHANNEL,null GLOBAL_SUBCHANNEL
      |from OPR_INPUT
    """.stripMargin)
