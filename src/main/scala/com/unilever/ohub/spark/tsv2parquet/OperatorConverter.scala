...
object OperatorConverter extends App {
  ...
  // TODO Rewrite this in code, so we automatically get all fields in OperatorRecord without explicitly listing them
  ...
  joinedRecordsDF.write.mode(Overwrite).partitionBy("COUNTRY_CODE").parquet(outputFile)
