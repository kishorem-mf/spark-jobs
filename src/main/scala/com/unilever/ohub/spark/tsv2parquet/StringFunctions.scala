...
//    Using General Email Regex (RFC 5322 Official Standard)
  ...
  def concatNames(firstName:String, lastName:String, emailAddress:String = ""):String = {
    try{
      ...
      if(firstName == null && lastName == null && emailWithoutNull == "") return null
      if(firstName == "" && lastName == "" && emailWithoutNull == "") return null
      if(firstName == null && emailWithoutNull == "") return lastName
      if(lastName == null && emailWithoutNull == "") return firstName

      val extractedNames = if(emailWithoutNull != "") emailWithoutNull.substring(0,emailWithoutNull.indexOf('@')) else ""
      if(firstName == "" && lastName == "" && extractedNames != "") {
