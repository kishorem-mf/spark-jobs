  ...
  val productsJsonFormat = countryLanguageListEmakina
    .map(
      listValues => {
        ...
        var newRange = 0
        breakable {
          for(i <- 1 to MAX by RANGE){
            if (getRestApiJsonString(createRestApiUrl(listValues._1, listValues._2, PRODUCTS, i, i + RANGE)) == "") break()
            newRange = i + RANGE
          }
        }
        if(newRange != 0) for(i <- 1 to newRange by RANGE) {
          jsonString += getRestApiJsonString(createRestApiUrl(listValues._1, listValues._2, PRODUCTS, i, i + RANGE))
        }
        jsonString match {
  ...
  val recipesJsonFormat = countryLanguageListEmakina
    .map(
      listValues => {
        ...
        breakable {
          for(i <- 1 to MAX by RANGE){
            if (getRestApiJsonString(createRestApiUrl(listValues._1, listValues._2, RECIPES, i, i + RANGE)) == "") break()
            newRange = i + RANGE
          }
        }
        if(newRange != 0) for(i <- 1 to newRange by RANGE) {
          jsonString += getRestApiJsonString(createRestApiUrl(listValues._1, listValues._2, RECIPES, i, i + RANGE))
        }
        jsonString match {
          case value: String if value.nonEmpty => jsonString
          case _ => ""
...
//  TODO create selection "queries" based on raw parquet needed for REC-O
  ...
  def createDataFrameFromJsonString(spark:SparkSession, jsonStrings:Array[String]): Dataset[String] = {
