...
class StorageSpec extends FunSpec with SparkJobSpec with Matchers {
  ...
  describe("creating URI") {
    it("should return a valid URI") {
      ...
      val created = storage.createSifuURL("NL", "nl", "products", 0, 1)
  ...
  describe("decoding json string") {
    it("should be parsed into valid case class") {
      val json: Either[circe.Error, SifuProductResponse] = storage.decodeJsonString[SifuProductResponse](jsonString)
  ...
  describe("response body") {
    it("should return ") {
      val response = storage.productsFromApi("NL", "nl", "products", 100, 2)
