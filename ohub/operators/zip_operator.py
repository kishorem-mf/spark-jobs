...
class ZipOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        logging.info(
            "Path to the File to Zip (path_to_file_to_zip): "
            + str(self.path_to_file_to_zip)
        )
        logging.info(
            "Path to save the Zip File (path_to_save_zip) : " + str(self.path_to_save_zip)
        )
            ...
            if is_file:
                ...
                for dirname, subdirs, files in os.walk(file_to_zip_name):
                    ...
                    for filename in files:
                        ...
                        logging.info(
                            "Writing '" + str(file_name_to_write) + "to zip file"
                        )
        ...
        logging.info(
            "Moving '" + str(zip_file_name) + "' to '" + str(self.path_to_save_zip) + "'"
        )
    ...
    def execute(self, context):
        ...
        with ZipFile(self.path_to_zip_file, "r") as zip_file:
            logging.info(
                ...
                + str(zip_file)
                + "' from path '"
                + str(self.path_to_zip_file)
                + "'"
            )
            logging.info(
                "Extracting all the contents to '"
