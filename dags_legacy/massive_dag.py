...
from dags.config import large_cluster_config, integrated_bucket, databricks_conn_id, spark_jobs_jar, ingested_bucket, \
    intermediate_bucket
from ohub.operators.databricks_operator import DatabricksSubmitRunOperator
from ohub_legacy.utils.airflow import SubPipeline, dag_default_args, DagConfig, GenericPipeline

dag_default_args.update(
    {
...
with DAG('ohub_massive', default_args=dag_default_args, schedule_interval=operators_dag_config.schedule) as dag:
    operators = (
        ...
        GenericPipeline(orders_dag_config, class_prefix='Order',
            ...
            .has_export_to_acm(acm_schema_name='ORDERS',
                               extra_acm_parameters=['--orderLineFile',
                                                     integrated_bucket.format(date="{{ ds }}", fn='orderlines')])
    ...
    merge_orders = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--orderInputFile',
                           ingested_bucket.format(date="{{ ds }}", channel='file_interface', fn=orders_entity),
                           '--previousIntegrated', integrated_bucket.format(date="{{ yesterday_ds }}", fn=orders_entity),
                           '--contactPersonInputFile', integrated_bucket.format(date="{{ ds }}", fn='contactpersons'),
                           '--operatorInputFile', integrated_bucket.format(date="{{ ds }}", fn='operators'),
                           '--outputFile', integrated_bucket.format(date="{{ ds }}", fn=orders_entity)]
        })
    ...
    merge_orderlines = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--orderLineInputFile',
                           ingested_bucket.format(date="{{ ds }}", channel='file_interface',
                                                  fn=orderlines_dag_config.entity),
                           '--previousIntegrated',
                           integrated_bucket.format(date="{{ ds }}", fn=orderlines_dag_config.entity),
                           '--outputFile', integrated_bucket.format(date="{{ ds }}", fn=orderlines_dag_config.entity)]
        })

    # OPERATORS

    operators_fuzzy_matching: SubPipeline = operators.construct_fuzzy_matching_pipeline(
        ingest_input=ingested_bucket.format(date="{{ ds }}", fn=operators_dag_config.entity, channel='*'),
    ...
    operators_join = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--matchingInputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_fuzzy_matched_delta'.format(operators_entity)),
                           '--entityInputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_delta_left_overs'.format(operators_entity)),
                           '--outputFile',
    ...
    operators_combine_to_create_integrated = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--integratedUpdated',
                           ...
                           '--newGolden',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_delta_golden_records'.format(operators_entity)),
                           '--combinedEntities',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_combined'.format(operators_entity))]
        }
    )
    ...
    operators_update_golden_records = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--inputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_combined'.format(operators_entity)),
                           '--outputFile',
                           integrated_bucket.format(date="{{ ds }}", fn=operators_entity)] + postgres_config
        }
    )
    ...
    merge_products = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--productsInputFile',
                           ingested_bucket.format(date="{{ ds }}", channel='file_interface', fn=products_entity),
                           '--previousIntegrated', integrated_bucket.format(date=two_day_ago, fn=products_entity),
                           '--outputFile', integrated_bucket.format(date="{{ ds }}", fn=products_entity)]
        })
    ...
    contactpersons_pre_processing = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--integratedInputFile',
                           ...
                           '--deltaInputFile',
                           ingested_bucket.format(date="{{ ds }}", fn=contactpersons_entity, channel='*'),
                           '--deltaPreProcessedOutputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_pre_processed'.format(contactpersons_entity))]
        }
    )
    ...
    contactpersons_exact_match_integrated_ingested = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--integratedInputFile',
                           ...
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_exact_matches'.format(contactpersons_entity)),
                           '--unmatchedIntegratedOutputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_unmatched_integrated'.format(contactpersons_entity)),
                           '--unmatchedDeltaOutputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_unmatched_delta'.format(contactpersons_entity))
                           ] + postgres_config
        }
    )
    ...
    contactpersons_join_fuzzy_matched = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--matchingInputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_fuzzy_matched_delta'.format(contactpersons_entity)),
    ...
    contactpersons_referencing = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--combinedInputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_combined'.format(contactpersons_entity)),
                           '--operatorInputFile', integrated_bucket.format(date="{{ ds }}", fn='operators'),
                           '--outputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_updated_references'.format(contactpersons_entity))]
        }
    )
    ...
    contactpersons_update_golden_records = DatabricksSubmitRunOperator(
        ...
        spark_jar_task={
            ...
            'parameters': ['--inputFile',
                           intermediate_bucket.format(date="{{ ds }}", fn='{}_updated_references'.format(contactpersons_entity)),
                           '--outputFile',
                           integrated_bucket.format(date="{{ ds }}", fn=contactpersons_entity)] + postgres_config
        }
    )
    ...
    fuzzy_matching_contactpersons: SubPipeline =  contactpersons.construct_fuzzy_matching_pipeline(
        ...
        integrated_input=intermediate_bucket.format(date="{{ ds }}", fn='{}_unmatched_integrated'.format(contactpersons_entity)),
        ingest_input=intermediate_bucket.format(date="{{ ds }}", fn='{}_unmatched_delta'.format(contactpersons_entity))
    )
