...
val CSV_NAME_PATTERN = Pattern.compile("UFS_([A-Z0-9_-]+)_(CHAINS|LOYALTY|OPERATORS|OPERATOR_ACTIVITIES|OPERATOR_CLASSIFICATIONS|CONTACTPERSONS|CONTACTPERSON_ACTIVITIES|CONTACTPERSON_CLASSIFICATIONS|CONTACTPERSON_SOCIAL|CONTACTPERSON_ANONYMIZATIONS|PRODUCTS|ORDERS|ORDERLINES|ORDERS_DELETED|ORDERLINES_DELETED|SUBSCRIPTIONS|QUESTIONS|ANSWERS|CAMPAIGN_OPENS|CAMPAIGN_CLICKS|CAMPAIGN_BOUNCES|CAMPAIGNS|CAMPAIGN_SENDS|CHANNEL_MAPPINGS)_([0-9]{14})");

def getModelAndSourceName(csvFileName: String) =  {
    var matcher = CSV_NAME_PATTERN.matcher(csvFileName.toUpperCase())
    if (matcher.matches()) {
        val modelName = matcher.group(2)
        val sourceName = matcher.group(1)
